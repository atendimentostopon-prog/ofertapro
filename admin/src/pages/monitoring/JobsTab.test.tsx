...
const h = vi.hoisted(() => ({ calls: [] as unknown[][],
  perms: { value: ['jobs.read'] as string[] },
  impl: (_r: string, action: string): Promise<unknown> => {
    ...
    if (action === 'run-job') return Promise.resolve({ ok: true, message: 'UPDATE 0', duration_ms: 40 });
...
beforeEach(() => { h.perms.value = ['jobs.read']; });
...
it('sem jobs.retry nao mostra "rodar agora"; com a permissao, clicar nao quebra', async () => {
  render(<MemoryRouter><JobsTab /></MemoryRouter>);
  await screen.findByText('expire_trials');
  expect(screen.queryByRole('button', { name: /rodar agora/i })).not.toBeInTheDocument();
});

it('com jobs.retry, "rodar agora" chama monitoring/run-job', async () => {
  h.perms.value = ['jobs.read', 'jobs.retry'];
  render(<MemoryRouter><JobsTab /></MemoryRouter>);
  await screen.findByText('expire_trials');
  await userEvent.click(screen.getByRole('button', { name: /rodar agora/i }));
  await waitFor(() => {
    const last = h.calls[h.calls.length - 1];
    expect(last[1]).toBe('run-job');
    expect((last[2] as { jobid?: number }).jobid).toBe(3);
  });
});
