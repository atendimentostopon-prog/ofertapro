...
🔗 [Garanta aqui]({link})`;
  ...
  renderTemplate(template: string, offer: any, userProfile: any, trackingLink: string, channelType?: string): string {
    ...
    // Se a linha contém o emoji correspondente antes da tag, substituímos a tag por uma versão sem o emoji redundante.
    // Ex: "❌ {preco_original_linha}" -> "❌ De: ~R$ 199,00~"
    // Variáveis que começam com emoji: originalPriceLine (❌), couponLine (🎟️), discountLine (🏷️), marketplaceLine (🛒), categoryLine (📂), imageLine (🖼️)
