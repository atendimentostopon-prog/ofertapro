...
export const DebugSupabase: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-graphite text-ink-inverse p-8 font-mono">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex justify-between items-center border-b border-graphite-600 pb-4">
          ...
          <button 
            ...
            className="px-4 py-2 bg-graphite-700 hover:bg-graphite-600 rounded text-sm font-semibold transition-colors"
          >
            Rodar Novamente
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-graphite-800 p-6 rounded-lg border border-graphite-600 space-y-4">
            <h2 className="text-sm font-bold text-ink-secondary uppercase tracking-wider">Resultados</h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span>React:</span>
                <span className="text-mint-400 font-bold">{results.react}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span>Vite Env URL:</span>
                <span className={results.envUrl.startsWith('OK') ? 'text-mint-400 font-semibold truncate max-w-[250px]' : 'text-danger'}>{results.envUrl}</span>
              </div>
              <div className="flex justify-between">
                <span>Vite Env Key:</span>
                <span className={results.envKey.startsWith('OK') ? 'text-mint-400 font-semibold' : 'text-danger'}>{results.envKey}</span>
              </div>
              <div className="flex justify-between">
                <span>Supabase Client:</span>
                <span className={results.client === 'OK' ? 'text-mint-400 font-bold' : 'text-danger'}>{results.client}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span>auth.getSession():</span>
                <span className={results.getSession.startsWith('SUCCESS') ? 'text-mint-400' : 'text-danger font-bold'}>{results.getSession}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span>profiles.select():</span>
                <span className={results.profilesSelect.startsWith('SUCCESS') ? 'text-mint-400' : 'text-danger font-bold'}>{results.profilesSelect}</span>
              </div>
              <div className="flex justify-between gap-4">
                <span>Health Check Fetch:</span>
                <span className={results.healthCheck.startsWith('SUCCESS') ? 'text-mint-400' : 'text-danger font-bold'}>{results.healthCheck}</span>
              </div>
            </div>
          </div>

          <div className="bg-graphite-800 p-6 rounded-lg border border-graphite-600 flex flex-col h-[300px]">
            <h2 className="text-sm font-bold text-ink-secondary uppercase tracking-wider mb-2">Logs de Execução</h2>
            <div className="flex-1 overflow-y-auto text-xs space-y-1 bg-slate-950 p-3 rounded border border-graphite-700">
              {logs.map((log, i) => (
        ...
        <div className="bg-graphite-800 p-6 rounded-lg border border-graphite-600 space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              className="px-4 py-2 bg-danger hover:bg-danger/85 rounded text-xs font-semibold transition-colors"
            >
              Limpar Sessão Local
            </button>
            <button
              ...
              className="px-4 py-2 bg-graphite-600 hover:bg-graphite-500 rounded text-xs font-semibold transition-colors"
            >
              Executar SignOut no Supabase
            </button>
            <a
              href="/login"
              className="px-4 py-2 bg-graphite-600 hover:bg-graphite-500 rounded text-xs font-semibold text-center transition-colors"
            >
              Ir para /login
            </a>
          </div>
