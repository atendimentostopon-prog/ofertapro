...
serve(async (req: Request) => {
  ...
  if (!subscription_id) return new Response("Missing subscription_id", { status: 400, headers: corsHeaders });
  ...
  if (!sub || sub.user_id !== user.id) {
    return new Response("Not found or forbidden", { status: 404, headers: corsHeaders });
  }
